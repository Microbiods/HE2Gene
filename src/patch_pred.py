...
from datasets.dataloader import PatchDataset
...
if __name__ == "__main__":
    ...
    # test dataset
    test_dataset = PatchDataset(image_root=args.root,
    ...
    model_path = './checkpoint/model_base.pt'
    ...
    with torch.no_grad():
        ...
        all_aux_count_pred = []
        all_tumor_pred = []
        for step, (name, image) in enumerate(test_dataloader):
            image = image.to(device)
            if args.average:
                count_pred, aux_count_pred, tumor_pred = model(image[0])
        ...
        # save to output
        all_count_pred.to_csv('./output/base/target_count_pred.csv')
        all_aux_count_pred.to_csv('./output/base/aux_count_pred.csv')
        all_tumor_pred.to_csv('./output/base/tumor_pred.csv')

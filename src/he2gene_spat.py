...
from datasets.gridloader import BreastCancerSTDataset
from utils.normalize import SetSeed
from models.resnet import AuxAnnotateSpatialResNet50
from models.losses import SpatialMSELoss
import os
...
def test(test_dataloader, model, criterion):
    ...
    with torch.no_grad():
        ...
        for step, (image, grid_images, tta_images, grid_tta_images, 
                   count, aux_count, 
                   nbr_image, nbr_count, nbr_aux_count, 
                   coord, nbr_coord, tumor, nbr_tumor, nbr_mask, 
                   pixel, patient, section) in enumerate(test_dataloader):
            
            
            count = count.to(device) 
            aux_count = aux_count.to(device)
            nbr_count = nbr_count.to(device)
            nbr_image = nbr_image.to(device) # [1, 8, 8, 3, 224, 224]
            tumor = tumor.to(device).to(torch.float32)
            nbr_tumor = nbr_tumor.to(device)
            ...
            count_pred, aux_count_pred, tumor_pred, *_ = model(image, nbr_image)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--aux', type=float, default=100.0, help='weight for aux loss')
    parser.add_argument('--tmr', type=float, default=1.0, help='weight for tmr loss')
    parser.add_argument('--spat', type=float, default=0.01, help='weight for spat loss')
    args = parser.parse_args()

    start_time = time.time()

    data_root = './data/breast/'
    save_root = './output/spat'
    ...
    for epoch in range(params['epochs']):
        ...
        for step, (image, count, aux_count, 
                       nbr_image, nbr_count, nbr_aux_count, 
                       coord, nbr_coord, tumor, nbr_tumor, 
                       nbr_mask, *_) in enumerate(train_dataloader):
            ...
            optimizer.zero_grad()
            count_pred, aux_count_pred, tmr_pred, nbr_count_pred, *_, = model(image, nbr_image)

            traget_loss = criterion(count_pred, count)
            aux_loss = criterion(aux_count_pred, aux_count)
            tmr_loss = criterion_tmr(tmr_pred, tumor)
            spat_loss = criterion_spat(count_pred, nbr_count_pred, tumor, nbr_tumor, nbr_mask)

            loss = traget_loss + args.aux * aux_loss + args.tmr * tmr_loss + args.spat * spat_loss
        ...
        with torch.no_grad():
            ...
            for step, (image, count, aux_count, 
                       nbr_image, nbr_count, nbr_aux_count, 
                       coord, nbr_coord, tumor, nbr_tumor, 
                       nbr_mask, *_) in enumerate(val_dataloader):
                count = count.to(device)
                image = image.to(device)
                count_pred, aux_count_pred, tumor_pred, *_ = model(image, nbr_image)
